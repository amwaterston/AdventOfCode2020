...
class Grid:
	...
	def setotherxyz(self, x, y, z):
		self.grid[(self.flip + 1) % 2][(x, y, z)] = "#"
		self.minx = min(self.minx, x-1)
		self.miny = min(self.miny, y-1)
		self.minz = min(self.minz, z-1)
		self.maxx = max(self.maxx, x+1)
		...
		for i in range(-1,2):
			for j in range(-1,2):
				for k in range(-1,2):
					...
					if (x+i, y+j, z+k) in self.grid[self.flip]:
						n += 1
		return n
		...
		for x in range(self.minx, self.maxx + 1):
			for y in range(self.miny, self.maxy + 1):
				for z in range(self.minz, self.maxz + 1):
					n = self.countNeighbours((x, y, z))
					#print (f"{x, y, z} has {n} active neighbours")
					if (n == 2 or n == 3) and (x, y, z) in self.grid[self.flip]:
						#print(f"(x, y, z) stays on")
						self.setotherxyz(x, y, z)
					elif n == 3 and (x, y, z) not in self.grid[self.flip]:
